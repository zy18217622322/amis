  ...
  <div class="login">
    <div class="content">
      ...
      <van-form @submit="onSubmit" @failed="onFailed">
        <van-field v-model.trim="loginInfo.username" name="username" clearable placeholder="input username" />
        <van-field v-model="loginInfo.password" :type="inputType" name="password" clearable placeholder="input password">
          <template #button>
...
const onSubmit = async () => {
  ...
  // startLoading();
  await Api.login();
  // const { code, result, message } = await Api.getList('123');
  // stopLoading();
  // do something
  // router.push('/index');
};
